...
export async function getHomePageContentStrapi() {
  const { data, loading, error } = await client.query({
    query: gql`
    query getAllEvents {
      eventsEvents (where: {slug: "home"}, limit: 1) {
        ${EventsModel}
      }
    }`
  });
...
export async function getRecentEventsStrapi() {
  const { data, loading, error } = await client.query({query: gql`
    query getAllEvents {
      eventsEvents (where: {endDate_gte: "${twoDaysBeforeNow.toISOString()}"}, limit: 1) {
        ${EventsModel}
      }
    }`
  });
...
export async function getSponsorshipPageContentStrapi() {
  const { data, loading, error } = await client.query({
    query: gql`
    query getAllLandingPageContent {
      eventsLandingPage (id: 1)  {
        ${LandingPageModel}
      }
    }`
  });
